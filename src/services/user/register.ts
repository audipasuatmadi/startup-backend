...
const registerUser = async (requestBody: RegisterRequestBody) => {
  try {
    ...
    const sanitizedErrorDetails: ImproperInputResponse[] = joiValidationError.details.map(
      (errorItem) => ({ [errorItem.context?.key!]: errorItem.type })
    );
    const reducedErrorDetails: ImproperInputResponse = sanitizedErrorDetails.reduce(
