...
describe('user registration service tests', () => {
  const mockedValidateUserRegisterBody = jest.spyOn(UserValidator, 'validateUserRegisterBody');
  ...
  it('should call validateUserRegisterBody with the proper arguments', async () => {
    await registerUser(dummyProperUserRegisterRequestBody)
    expect(mockedValidateUserRegisterBody).toHaveBeenCalledTimes(1)
    expect(mockedValidateUserRegisterBody).toHaveBeenLastCalledWith(dummyProperUserRegisterRequestBody)
  })

  it('should return the right response object if the request doesn\'t pass validation', async () => {
    const improperResponse = await registerUser(dummyImproperUserRegisterRequestBody) as RegisterValidationFailed
    expect(mockedValidateUserRegisterBody).toHaveBeenCalledTimes(1)
    expect(mockedValidateUserRegisterBody).toHaveBeenCalledWith(dummyImproperUserRegisterRequestBody)
